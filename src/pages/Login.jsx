    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
        ...
        <form onSubmit={hook.handleLogin} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <input type={hook.showPassword ? 'text' : 'password'} required className="w-full px-4 py-2 pr-12 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-600 outline-none" placeholder="••••••••" value={hook.senha} onChange={(e) => hook.setSenha(e.target.value)} />
        ...
        <div className="mt-6 text-center text-sm text-slate-600 border-t border-slate-100 pt-4">
          Novo por aqui? <button onClick={() => hook.navigate('/register')} className="text-blue-600 hover:underline font-semibold">Cadastre-se aqui</button>
        </div>
      </div>
    </div>
  );
