...
export default function TechMyTicketsTableRow({ ticket, equipments, isExpanded, onToggleExpand }) {
    ...
    <tr className="hover:bg-slate-50 transition align-top">
      ...
      <td className="py-4 px-4 pt-5 text-sm font-medium text-slate-500">
        <div className="flex flex-col gap-1 leading-tight whitespace-nowrap">
          ...
          <span>Fechamento: {dataFechamento ? new Date(dataFechamento).toLocaleString('pt-BR') : (ticket.status_chamado === 'Aguardando Confirmação' ? 'Aguardando usuário' : 'Em aberto')}</span>
        </div>
      </td>
