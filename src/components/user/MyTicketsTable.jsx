        ...
        <table className="w-full text-left text-sm text-slate-600">
          ...
          <tbody>
            {[...tickets].sort((a,b) => b.id - a.id).map(ticket => {
              ...
              return (
                <tr key={ticket.id} className={`border-b align-top transition ${ticket.status_chamado === 'Cancelado' ? 'bg-slate-50/50 opacity-70' : 'hover:bg-slate-50'}`}>
                  ...
                  <td className="py-3 px-3 pt-4">
                    <span className="font-bold text-blue-600 text-sm block">{matchedEq ? matchedEq.patrimonio : `ID: ${ticket.equipment_id}`}</span>
                  </td>
                  <td className="py-3 px-3 max-w-xs pt-4">
                    ...
                    {ticket.descricao_problema.length > 50 && (
                      <button onClick={() => toggleExpandirChamado(ticket.id)} className="text-xs text-blue-600 hover:underline font-bold block mt-1 cursor-pointer">
                    ...
                    {ticket.resolucao_ti && (
                      <div className={`mt-3 p-2.5 border rounded-lg text-sm shadow-sm ${
                        ...
                        <strong className="block mb-1 font-bold">{ticket.status_chamado === 'Cancelado' ? 'Histórico de Cancelamento:' : 'Resposta do Suporte:'}</strong>
                  ...
                  <td className="py-3 px-3 text-center pt-4">
                    {ticket.status_chamado === 'Cancelado' ? (
                      <span className="text-xs text-slate-400 italic font-medium">Encerrado</span>
                    ) : nomeTecnico ? (
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 text-amber-700 rounded-full border border-amber-200 text-xs font-bold">
                  ...
                  <td className="py-3 px-3 text-center pt-4">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-bold border ${
                      ...
                      ticket.status_chamado === 'Cancelado' ? 'bg-slate-200 text-slate-600 border-slate-300' : // NOVO: Cor do Cancelado
                  ...
                  {/* ATUALIZADO: Ações ganham o botão de cancelar se o chamado estiver em aberto */}
                  <td className="py-3 px-3 text-center pt-4">
